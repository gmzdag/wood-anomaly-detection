...
Original file is located at
    https://colab.research.google.com/drive/1KKzqGUqnDjRhdk9maMipTUXPB6h-Ttyl

# FastFlow

Bu notebook'ta, ahşap yüzeylerdeki anomalileri tespit etmek için bir *FastFlow* modeli geliştirilecek ve eğitilecektir.
...
from torchmetrics.classification import BinaryF1Score, BinaryRecall,BinaryAccuracy

"""## 3. Veriseti Yapılandırması

Anomalib'in veri modüllerini kullanarak veri setini FastFlow modeline uygun hale getirme. `Folder` modülü kullanılarak klasör yapısına dayalı bir veri seti tanımlanır.
...
threshold_metric = F1AdaptiveThreshold(fields=["pred_score", "gt_label"])
...
"""## 5. Model Yapılandırılması ve Eğitimi

FastFlow modelini tanımlanamsı ve PyTorch Lightning tabanlı `Engine` kullanarak eğitim sürecinin yönetilmesi.

**Model Parametreleri:**
- `backbone`: Modelin temelini oluşturan önceden eğitilmiş ağ ('wide_resnet50_2').
- `pre_trained`: Backbone için önceden eğitilmiş ağırlıkların kullanılıp kullanılmayacağı.
- `flow_steps`: FastFlow modelindeki akış adımı sayısı.
...
- `default_root_dir`: Eğitim çıktıları için varsayılan kök dizin.
- `log_every_n_steps`: Loglama sıklığı.
"""
...
logger = CSVLogger(save_dir="/content/drive/MyDrive/FastFlow", name = "fastflow_logs")

"""### 5.1 20 Epoch İle Eğitim"""

engine = Engine(max_epochs = 20,
                logger = logger,
                default_root_dir='/content/FastFlow_training_output',
                log_every_n_steps=1)
...
"""### 5.2 30 Epoch ile Model Eğitimi"""
...
"""### 5.3 50 Epoch İle Model Eğitimi"""

engine = Engine(max_epochs = 50, logger = logger)
engine.fit(model, datamodule=datamodule)

test_results = engine.test(model, datamodule=datamodule)
print("Test Sonuçları:", test_results)

save_path = "/content/drive/MyDrive/FastFlow/fastflow_model.pth"
torch.save(model.state_dict(), save_path)
print(f"Model ağırlıkları '{save_path}' dosyasına kaydedildi.")

engine = Engine(max_epochs = 100, logger = logger)
engine.fit(model, datamodule=datamodule)
